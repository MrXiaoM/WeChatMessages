...
HANDLE InjectDll(DWORD pid, LPCWSTR dllPath, HMODULE *injectedBase)
{
    ...
    if (k32 == NULL) {
        LOG_WARN("[InjectDll] 获取 kernel32 失败");
    ...
    if (!libAddr) {
        LOG_WARN("[InjectDll] 获取 LoadLibaray 失败");
...
bool EjectDll(HANDLE process, HMODULE dllBase)
{
    ...
    HMODULE k32 = GetModuleHandle(L"kernel32.dll");
    if (k32 == NULL) {
        LOG_WARN("[EjectDll] 获取 kernel32 失败");
    ...
    if (!libAddr) {
        LOG_WARN("[EjectDll] 获取 FreeLibrary 失败");
    ...
    if (hThread == NULL) {
        LOG_WARN("[EjectDll] FreeLibrary 调用失败");
...
static UINT64 GetFuncOffset(LPCWSTR dllPath, LPCSTR funcName)
{
    ...
    if (dll == NULL) {
        err = GetLastError();
        LOG_WARN("[GetFuncOffset] LoadLibaray 获取 DLL 失败，err={}", to_string(err));
